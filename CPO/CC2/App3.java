...
class MagicRoom extends Room {
    private int immuneCount = 3;
    
    @Override
    void action(Maze maze) {
        System.out.println("Thy shall prevail");
        maze.prisoner.setState(new StrongState());
        immuneCount--;
        if (immuneCount == 0) {
            maze.currentRoom = maze.currentRoom.getNextRoom(Direction.NORTH);
            maze.prisoner.setState(new NormalState());
        }
    }
}
interface State {
